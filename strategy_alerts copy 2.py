    ...
    try:
        with open('config.json', 'r') as f:
            ...
            return {
                ...
                'end_h': default_range.get('end', {}).get('h', 14),
                'end_m': default_range.get('end', {}).get('m', 00),
...
def check_alerts():
    ...
    for symbol in active_symbols:
        try:
            # 1. Fetch Data
            df_1min = fetch_1min_candles(symbol, limit=500)
            if df_1min.empty:
                continue
            ...
            if cond_price and cond_cvd:
                state_key = (symbol, TIMEFRAME, 'custom_strategy')
            ...
            frvp_range_str = f"{int(frvp_config['start_h']):02d}:{int(frvp_config['start_m']):02d}-{int(frvp_config['end_h']):02d}:{int(frvp_config['end_m']):02d}"
            logger.info(f"    Scanning {symbol} ({TIMEFRAME}) | Range {frvp_range_str} | Price={current_close:.2f} > VAH={frvp_vah:.2f}? | CVD={current_cvd:.0f} > Ref={cvd_ref_level:.0f}? | POC={frvp_poc:.2f} | VAL={frvp_val:.2f}")
...
def print_startup_summary():
    ...
    for symbol in SYMBOLS:
        try:
            # Fetch just enough data to calculate references
