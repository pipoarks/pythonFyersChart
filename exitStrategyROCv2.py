...
def exit_position(symbol, side, qty, order_id):
    ...
    # ID format for position square-off: symbol-productType
    pos_id = f"{symbol}-INTRADAY"
    payload = {"id": pos_id}
...
def check_and_report_roc(symbol, side, qty, order_id, now, is_startup=False):
    """
    Fetches latest candle data, calculates ROC, prints it, and checks exit criteria.
    If is_startup=True, it only prints without executing exits.
    """
    try:
        ...
        # 3. Find the latest COMPLETED candle
        # now_ts is current epoch. Completed candle is the one that finished just before the current slot.
        # e.g., at 13:04, latest completed candle is 12:55-13:00 (starts at 12:55).
        now_ts = now.timestamp()
        target_candle_start = (int(now_ts) // 300) * 300 - 300
        target_time_in_data = target_candle_start + 19800 

        candles = processed["candles"]
        last_candle = None
        # We look back for the nearest candle that started at or before our target time
        for c in reversed(candles):
            if c["time"] <= target_time_in_data:
                last_candle = c
                break

        if not last_candle:
            print(f"ℹ️ [{symbol}] Could not find completed candle for ROC calculation.")
            return
        ...
        # PRINT TO CONSOLE
        prefix = "[STARTUP]" if is_startup else "[MONITOR]"
        print(f"📊 [{ist_now}] {prefix} {symbol} ({side}) | ROC: {roc_val:.4f} @ {ist_candle_time}")

        # 4. Check Exit Criteria (Only if NOT startup)
        if not is_startup:
...
def monitor_roc_exit():
    ...
    try:
        ...
        print(f"📋 [{ist_now}] [INIT] Fetching open positions for startup report...")
        
        with sqlite3.connect(DB_PATH, timeout=30) as conn:
            cur = conn.cursor()
            cur.execute("""
                SELECT symbol, side, qty, order_id FROM positions 
                WHERE is_squared_off = 0 AND entry_time LIKE ?
            """, (f"{today_date}%",))
            active_trades = cur.fetchall()
        
        if active_trades:
            for symbol, side, qty, order_id in active_trades:
                check_and_report_roc(symbol, side, qty, order_id, now, is_startup=True)
            print(f"✅ [{ist_now}] [INIT] Startup ROC report completed.\n")
        else:
            print(f"ℹ️ [{ist_now}] [INIT] No active positions found for today.")
    except Exception as e:
        print(f"❌ Error during startup ROC report: {e}")
    ...
    while True:
        try:
            ...
            if now.minute % 5 == 0 and now.minute != last_check_minute:
                ...
                # Fetch active positions for the current day
                with sqlite3.connect(DB_PATH, timeout=30) as conn:
                    cur = conn.cursor()
                    cur.execute("""
                        SELECT symbol, side, qty, order_id FROM positions 
                ...
                if active_trades:
                    for symbol, side, qty, order_id in active_trades:
                        check_and_report_roc(symbol, side, qty, order_id, now, is_startup=False)
                    
                    print(f"✅ [{ist_now}] [INTERVAL DONE] ROC check completed for all symbols.\n")
                else:
                    print(f"ℹ️ [{ist_now}] No active positions found for today.")
                    
        except Exception as e:
            print(f"❌ Error in ROC Monitor Loop: {e}")
...
if __name__ == "__main__":
    try:
        print(f"🚀 [{get_ist_time()}] Connecting to Fyers (v2)...")
        fyers.connect()
