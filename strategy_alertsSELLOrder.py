...
# Prevent propagation to root logger to avoid duplicate logging in console/general log
...
# Stream Handler for Alerts (to see them in console without duplication)
alert_sh = logging.StreamHandler()
alert_sh.setFormatter(logging.Formatter('%(asctime)s [ALERT] %(message)s'))
...
def calculate_cvd_reference(df):
    """
    Calculates the CVD Reference Level (LOW of first N candles of the current day).
    For SELL strategy, we track the lowest CVD level from the first N candles.
    """
    if df.empty or 'cvd_l' not in df.columns:
        return None
...
def check_alerts():
    ...
    for symbol in active_symbols:
        try:
            # Check if symbol already exists in active_positions.db for today
            try:
                conn = sqlite3.connect(DB_PATH)
                cur = conn.cursor()
                cur.execute("SELECT 1 FROM positions WHERE symbol = ? AND entry_time LIKE ?", (symbol, f"{today_str}%"))
                exists = cur.fetchone()
                conn.close()
                
                if exists:
                    if symbol not in STOPPED_SYMBOLS:
                        logger.info(f"⏭️  Skipping {symbol}: Already present in DB for today.")
                        STOPPED_SYMBOLS.add(symbol)
                    continue
            except Exception as db_err:
                logger.error(f"⚠️ DB Error checking {symbol}: {db_err}")
            ...
            cond_price = current_close < frvp_val
            ...
            if cond_price and cond_cvd:
                state_key = (symbol, TIMEFRAME, 'sell_strategy')
                ...
                if alert_state.get(state_key) != timestamp:
                    ...
                    alert_logger.info(f"    ✨ LTP:   {current_close:.2f}")
                    ...
                    # POST Order Request
                    try:
                        url = "http://localhost:4000/postOrder"
                        payload = {
                            "ticker": symbol,
                            "action": "SELL",
                            "close": current_close
                        }
                        logger.info(f"📡 Sending order request for {symbol} to {url}...")
                        response = requests.post(url, json=payload, timeout=5)
                        logger.info(f"📩 Server Response: {response.status_code} - {response.text}")
                    except Exception as e:
                        logger.error(f"❌ Failed to send order request for {symbol}: {e}")
            else:
                # Debug line to see why it didn't trigger
                reason = []
                if not cond_price: reason.append(f"Price {current_close:.2f} >= VAL {frvp_val:.2f}")
            ...
            logger.info(f"    {symbol} | CVD={current_cvd:.0f} vs Ref={cvd_ref_level:.0f} | Close={current_close:.2f} vs VAL={frvp_val:.2f}")
...
def print_startup_summary():
    ...
    for symbol in SYMBOLS:
        try:
            ...
            val_text = f"{frvp_res['val']:.2f}" if frvp_res else "N/A"
            
            ref_text = f"{cvd_ref:.0f}" if cvd_ref is not None else "N/A"
            
            logger.info(f"✅ {symbol:<20} | CVD Ref (1st {cvd_config['refCandles']} c): {ref_text:>6} | FRVP VAL: {val_text:>8}")
