...
def monitor_cmf_exit():
    """Background loop to check CMF conditions for active intraday symbols every 2 minutes."""
    print(f"🚀 [{get_ist_time()}] CMF Exit Monitor Started (2-min polling)...")
    
    while True:
        try:
            today_str = datetime.now().strftime('%Y-%m-%d')
            
            # Fetch active positions opened today
            with sqlite3.connect(DB_PATH, timeout=30) as conn:
                cur = conn.cursor()
                cur.execute("""
            ...
            if not active_trades:
                # print(f"ℹ️ [{get_ist_time()}] No active intraday trades to monitor.")
                pass
            else:
                for symbol, side, qty, order_id in active_trades:
                    print(f"🔍 Checking exit for {symbol}...")
                    # 1. Fetch 1-min candles (DESC to get LATEST, then sorted inside provider)
                    ...
                    # Filter for TODAY'S data only to prevent stale CMF calculation
                    ...
                    if df_1min.empty or len(df_1min) < 20:
                        print(f"⚠️ Not enough data for {symbol} today. Rows: {len(df_1min)}")
                        # Not enough data for today yet (need at least 20 mins for CMF-20)
                        continue

                    # 2. Process for 5min CMF
                    processed = get_processed_candles(df_1min, tf="5min", cmf_len=16)
                    if not processed or not processed.get("candles") or len(processed["candles"]) < 2:
                        continue
                    
                    # Check the last closed 5-min candle
                    candles = processed["candles"]
                    last_candle = candles[-1]
                    timestamp = last_candle["time"]
                    cmf_val = last_candle.get("cmf")
                    ...
                    print(f"📊 [{ist_now}] [MONITOR] {symbol} ({side}) | CMF: {cmf_val:.4f} @ {ist_candle_time}")

                    # 3. Check Exit Criteria
                    if side == "LONG" and cmf_val < -0.05:
                        trigger_msg = f"🚨 [EXIT TRIGGER] {symbol} (LONG) | CMF: {cmf_val:.4f} < -0.05 | Time: {ist_candle_time}"
                        ...
                        trigger_msg = f"🚨 [EXIT TRIGGER] {symbol} (SHORT) | CMF: {cmf_val:.4f} > 0.05 | Time: {ist_candle_time}"
                        print(f"[{ist_now}] {trigger_msg}")
                        exit_logger.info(trigger_msg)
                        exit_position(symbol, side, qty, order_id)
                        
        except Exception as e:
            print(f"❌ Error in CMF Monitor: {e}")
            
        time.sleep(120) # 2-minute interval

# Start background monitor
threading.Thread(target=monitor_cmf_exit, daemon=True).start()
